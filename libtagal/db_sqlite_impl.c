...
#define CREATE_TABEL_STMT	\
	"create table tags(id INTEGER PRIMARY KEY, "	\
	"tag TEXT UNIQUE, nr_ref INTEGER DEFAULT 0);"	\
	"create table files(id INTEGER PRIMARY KEY, "	\
	"name TEXT, path TEXT UNIQUE);"	\
	"create table tag_file_map(id INTEGER PRIMARY KEY, "	\
	"tid INTEGER, fid INTEGER);"
...
int tagal_db_sqlite_init(void **lite, const char *arg, int if_create)
{
	...
	strcpy(str, arg);
	h->filename = str;
	if(rc = sqlite3_open_v2(arg, &(h->db), SQLITE_OPEN_READWRITE, NULL)) {
		if(if_create) {
			if(rc = sqlite3_open_v2(arg, &(h->db), 
				SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, 
				NULL)) {
				ERROR(("sqlite connect error. errno: %d", rc));
				return DBERR_CONN_ERR;
			}
			if(rc = sqlite3_exec(h->db, CREATE_TABEL_STMT, 
				NULL, NULL, &str)) {
				ERROR(("sqlite create error: %d- %s", rc, str));
				return DBERR_CONN_ERR;
			}
		} else {
			ERROR(("sqlite connect error. errno: %d", rc));
